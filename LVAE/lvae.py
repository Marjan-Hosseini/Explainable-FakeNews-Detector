...
def train_vae(run_info, top_folder):
    ...
    model.autoencoder.fit(x=text, y={'decoded_txt': np.expand_dims(text, -1), 'fnd_output': label}, batch_size=128,
...
def extract_features(run_info, top_folder, filtered_data_df, data_name, col_label, output_label='label'):
...
def new_extract_features(run_info, top_folder, data_name):
    print('Extracting VAE features for', data_name)
    main_folder = run_info['main_folder']
    sequence_length = run_info['sequence_length']
    latent_dim = run_info['latent_dim']
...
def mvae_classifier(run_info, top_folder):
    main_folder = run_info['main_folder']
    sequence_length = run_info['sequence_length']
    latent_dim = run_info['latent_dim']
    reg_lambda = run_info['reg_lambda']
    ...
    # load the model
    model.autoencoder.load_weights(main_folder + 'model_weights.hdf5')
    classifier = model.fnd

    # lossWeights = {"decoded_txt": 1.0, "fnd_output": 1.0}
    classifier.compile(optimizer=Adam(1e-5), loss='binary_crossentropy', metrics=['accuracy'])
    classifier.fit(vae_features_train, y_train, batch_size=64, epochs=epoch_no, verbose=1)

    classifier.save_weights(main_folder + 'classifier_weights.hdf5')
    classifier_history = classifier.history.history

    with open(main_folder + 'classifier_history.pickle', 'wb') as handle:
        pkl.dump(classifier_history, handle)

    pred = classifier.predict(vae_features_train)
    pred[pred > 0.5] = 1
    pred[pred <= 0.5] = 0

    pred_te = classifier.predict(vae_features_test)
    ...
    vae_results.to_csv(main_folder + 'vae_classifier_trained.csv')
