...
def pre_process_data_2(True_address, label):
    data_df = pd.read_csv(True_address)
...
def new_preprocess_and_filter(df, dataset_name, col_label='text'):
    if dataset_name == 'ISOT':
        ...
        for idx in range(len(df)):
            ...
            tweet_text_list = new_process_text(raw_tweet)
...
def preprocess_and_filter(df, dataset_name, col_label='text'):
    if dataset_name == 'ISOT':
        ...
        for idx in range(len(df)):
            ...
            if len(tweet_text_list) == 0 or tweet_text_list == ['0', '0']:
                filtered_idx.append(idx)
                new_df.loc[idx, col_label] = raw_tweet
            else:
                tweet_text = ' '.join(tweet_text_list)
                if detect(tweet_text) != 'en':
                    filtered_idx.append(idx)
                new_df.loc[idx, col_label] = tweet_text
        ...
        for idx in range(len(df)):
            ...
            if len(tweet_text_list) == 0 or tweet_text_list == ['0', '0']:
                ...
                if detect(tweet_text) != 'en':
                    filtered_idx.append(idx)
                new_df.loc[idx, :] = tweet_text, label
...
def get_embeds(folder, data_df, embed_dim, col_label='text'):
    ...
    for idx in range(len(data_df)):
        ...
        # tweet_words = tokenize(tweet_text)
...
def pre_process_dataset(folder, data_df_all, dataset_name, target_column, word2vec_dim, sequence_len, test_size=0.1):
    if dataset_name == 'ISOT':
        ...
        temp_x_train, temp_x_test, temp_y_train, temp_y_test = train_test_split(data_df_all, data_df_all['label'],
                                                            test_size=test_size, random_state=2021,
                                                            shuffle=True, stratify=data_df_all['label'])
        ...
        load_data(folder, temp_x_train, dataset_name, sequence_len, phase='train', embed_dim=word2vec_dim, col_label=target_column,
                  output_label='label')
        load_data(folder, temp_x_test, dataset_name, sequence_len, phase='test', embed_dim=word2vec_dim, col_label=target_column,
                  output_label='label')
        ...
        real_no = len(data_df_all[data_df_all['label'] == 'real'])
        fake_no = len(data_df_all[data_df_all['label'] == 'fake'])
        print('Number of negative articles:', real_no, '\nNumber of positive articles:', fake_no)
    ...
    if dataset_name == 'ISOT':
        ...
        if not os.path.exists(dataset_address + dataset_name + '/negative_semi_filtered_' + target_column +
                              '.csv') or not os.path.exists(dataset_address + dataset_name + '/positive_semi_filtered_'
                                                            + target_column + '.csv'):
            real_address = dataset_address + dataset_name + '/negative_articles.csv'
            fake_address = dataset_address + dataset_name + '/positive_articles.csv'
            data_df_real = pd.read_csv(real_address)
            data_df_fake = pd.read_csv(fake_address)
            filtered_data_df_real, _ = preprocess_and_filter(data_df_real, dataset_name, col_label=target_column)
            filtered_data_df_fake, _ = preprocess_and_filter(data_df_fake, dataset_name, col_label=target_column)
            ...
            real_address = dataset_address + dataset_name + '/negative_semi_filtered_' + target_column + '.csv'
            fake_address = dataset_address + dataset_name + '/positive_semi_filtered_' + target_column + '.csv'
            filtered_data_df_real = pd.read_csv(real_address)
            filtered_data_df_fake = pd.read_csv(fake_address)
...
def new_prepare_data(run_info, top_folder, dataset_address):
    target_column = run_info['target_column']
    sequence_length = run_info['sequence_length']
    dataset_name = run_info['dataset_name']
    word2vec_dim = run_info['word2vec_dim']
    ...
    if dataset_name == 'ISOT':
        ...
        data_df_real = pd.read_csv(real_address)
        data_df_fake = pd.read_csv(fake_address)
        filtered_data_df_real, _ = new_preprocess_and_filter(data_df_real, dataset_name, col_label=target_column)
        filtered_data_df_fake, _ = new_preprocess_and_filter(data_df_fake, dataset_name, col_label=target_column)
        filtered_data_df_real['label'] = 'real'
        filtered_data_df_fake['label'] = 'fake'
        filtered_data_df_all = filtered_data_df_real.append(filtered_data_df_fake, ignore_index=True)
        ...
        address = dataset_address + dataset_name + '/COVID_Fake_News_Data.csv'
        data_df = pd.read_csv(address)
        filtered_data_df, _ = new_preprocess_and_filter(data_df, dataset_name, col_label=target_column)
...
def make_run_info(top_folder, dataset_name, latent_dim, epoch_no, n_topics, n_iter, word2vec_dim):
    ...
    if dataset_name == 'ISOT':
        ...
        sequence_length = 53
        target_column = 'Title'  # 'Abstract' (465), 'Title'(53)
